  ...
  const displayQuestions = questions.map((question, index) => (
    <article className="p-3 text-left flex flex-col gap-6 bg-white rounded-xl w-64 h-72" key={`${question.name}-${index}`}>
      <div className='flex gap-24'>
        ...
        <button 
        className="flex justify-center items-center border border-indigo-400 rounded-full w-7 h-7 text-indigo-400 text-2xl mt-1 pb-1.5 " type="button" onClick={() => addQuestionsToAssessment(question)}
        >
          &#43;
        </button>
      </div>
